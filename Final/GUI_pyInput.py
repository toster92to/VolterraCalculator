...
from PyQt5.QtCore import QSize, QThread, pyqtSignal, QProcess
import matplotlib.pyplot as plt

from Volterra_In_Out_7 import VolterraResponse
from Volterra_File_Operations import readKernels, writeKernels
from Volterra_Operations_noncausal import wraparoundToOrdered, make_symmetrical
...
class VolterraCalculator(QWidget):
	...
	def create_system(self):
		...
		#Default Settings here - total length
		default_length1=500
		default_length2=150
		default_length3=50
		default_length4=15#27
		default_length5=15#27
		
		#warning: default_length must be >= 2default_nlen
		
		#negative index lengths
		default_nlen=np.array([12,12,12,12,12])
		...
		new_system.set_kernels(
			0,
			np.zeros(default_length1),
			np.zeros(shape=(default_length2, default_length2)),#40
			np.zeros(shape=(default_length3, default_length3, default_length3)),#40
			np.zeros(shape=(default_length4, default_length4, default_length4, default_length4)),#20
			np.zeros(shape=(default_length5, default_length5, default_length5, default_length5, default_length5)),#10
			default_nlen
		)
		...
		try:
			...
			new_system=make_symmetrical(wraparoundToOrdered(new_system))
	...
	def invertKernels(self):
		...
		try:
			...
			self.worker = Worker(vo5.invertKernels, system,
								 None, output_system_name, self)
			self.worker.finished.connect(self.on_worker_finished)
	...
	def plot_kernels(self):
		...
		if selected_item:
			...
			try:
				#print(system.nLen1)
				vps.plot1(system, 0, system.len1+system.nLen1)
				vps.plot2(system, 0, system.len2+system.nLen2)
				vps.plot3(system, 0, system.len3+system.nLen3)
